...
class PolynomialBasisFunction(BasisFunction):
    ...
    basisfunction_name : str
        The name of the basis function type, i.e., 'Polynomial'.
    ...
    basisfunction_deriv1(new_data)
        ...
        Note that all entries in the first row are 1.
    ...
    basisfunction_deriv2(new_data)
        ...
        Note that all entries in the first row are 0 and that all entries in the second row are 2.
    ...
    def basisfunction_eval(self, new_data):
        ...
        new_data : numpy.ndarray
            A new data array at which the polynomial basis function is to be evaluated.

        Returns
        -------
        numpy.ndarray
            An array of size (self.degree, self.new_data.shape[0])
            whose (i, j)-entry is new_data[j] ** i.
    ...
    def basisfunction_deriv1(self, new_data):
        ...
        ----------
        new_data : numpy.ndarray
            A new data array at which the first derivative of the polynomial basis function is to be evaluated.
    ...
    def basisfunction_deriv2(self, new_data):
        ...
        ----------
        new_data : numpy.ndarray
            A new data array at which the second derivative of the polynomial basis function is to be evaluated.
...
class GaussianBasisFunction(BasisFunction):
    ...
    basisfunction_name : str
        The name of the basis function type, i.e., 'Gaussian'.
    ...
    basisfunction_eval(new_data)
        ...
        where X_i is the i-th row in landmarks, and Y_j is the j-th row in new_data.
    ...
    basisfunction_deriv2(new_data)
        ...
        The result is an array of size (self.landmarks.shape[0], self.new_data.shape[0])
        whose (i, j)-entry is
        partial_2^2 k (X_i, Y_j) = exp(- (X_i - Y_j)^2 / (2 * bw ^ 2)) * ((X_i - Y_j) ^ 2 - bw ^ 2) / bw ^ 4,
        where X_i is the i-th row in landmarks, and Y_j is the j-th row in new_data.
    ...
    def basisfunction_eval(self, new_data):
        ...
        ----------
        new_data : numpy.ndarray
            A new data array at which the Gaussian basis function is to be evaluated.

        Returns
        -------
        numpy.ndarray
            An array of size (self.landmarks.shape[0], self.new_data.shape[0])
            whose (i, j)-entry is k (X_i, Y_j) = exp(- (X_i - Y_j)^2 / (2 * bw ^ 2)),
            where X_i is the i-th row in landmarks, and Y_j is the j-th row in new_data.
    ...
    def basisfunction_deriv1(self, new_data):
        ...
        ----------
        new_data : numpy.ndarray
            A new data array at which the first derivative of the Gaussian basis function is to be evaluated.
    ...
    def basisfunction_deriv2(self, new_data):
        ...
        ----------
        new_data : numpy.ndarray
            A new data array at which the second derivative of the Gaussian basis function is to be evaluated.
...
class RationalQuadraticBasisFunction(BasisFunction):
    ...
    basisfunction_name : str
        The name of the basis function type, i.e., 'RationalQuadratic'.
    ...
    def basisfunction_eval(self, new_data):
        ...
        ----------
        new_data : numpy.ndarray
            A new data array at which the rational quadratic basis function is to be evaluated.

        Returns
        -------
        numpy.ndarray
            An array of size (self.landmarks.shape[0], self.new_data.shape[1])
            whose (i, j)-entry is 1 / (1 + ((X_i - Y_j) ^ 2 / (bw ^ 2))),
            where X_i is the i-th row in landmarks, and Y_j is the j-th row in new_data.
    ...
    def basisfunction_deriv1(self, new_data):
        ...
        ----------
        new_data : numpy.ndarray
            A new data array at which the first derivative of the rational quadratic basis function is to be evaluated.
    ...
    def basisfunction_deriv2(self, new_data):
        ...
        ----------
        new_data : numpy.ndarray
            A new data array at which the second derivative of the rational quadratic basis function is to be evaluated.
            
        Returns
        -------
        numpy.ndarray
            An array of size (self.landmarks.shape[0], self.new_data.shape[0]) whose (i, j)-entry is
            4 * (1+(X_i-Y_j)^2/(bw^2))^(-3) * (X_i-Y_j)^2/bw^4 - 2 * (1+(X_i-Y_j)^2/(bw^2))^(-2)/bw^2,
            where X_i is the i-th row in landmarks, and Y_j is the j-th row in new_data.
...
class LogisticBasisFunction(BasisFunction):
    ...
    basisfunction_name : str
        The name of the basis function type, i.e., 'Logistic'.
    ...
    basisfunction_deriv2(new_data)
        Returns the evaluations of the second derivative of the logistic basis function
        with respect to the second argument at new_data.
        The result is an array of size (self.landmarks.shape[0], self.new_data.shape[1])
        whose (i, j)-entry is the second partial derivative of the logistic basis function
        with respect to the second argument evaluated at (X_i, Y_j),
        where X_i is the i-th row in landmarks, and Y_j is the j-th row in new_data.
    ...
    def basisfunction_eval(self, new_data):
        ...
        ----------
        new_data : numpy.ndarray
            A new data array at which the logistic basis function is to be evaluated.
    ...
    def basisfunction_deriv1(self, new_data):
        ...
        ----------
        new_data : numpy.ndarray
            A new data array at which the first derivative of the logistic basis function is to be evaluated.
    ...
    def basisfunction_deriv2(self, new_data):
        ...
        ----------
        new_data : numpy.ndarray
            A new data array at which the second derivative of the logistic basis function is to be evaluated.
...
class TriweightBasisFunction(BasisFunction):
    ...
    basisfunction_name : str
        The name of the basis function type, i.e., 'Triweight'.
    ...
    basisfunction_eval(new_data)
        ...
        The result is an array of size (self.landmarks.shape[0], self.new_data.shape[1])
        whose (i, j)-entry is (1 - (X_i - Y_j)^2 / self.bw^2)^3 if |X_i - Y_j| is less than or equal to self.bw
        and is 0 otherwise,
        where X_i is the i-th row in landmarks, and Y_j is the j-th row in new_data.
        
    basisfunction_deriv1(new_data)
        Returns the evaluations of the first derivative of the triweight basis function
        with respect to the second argument at new_data.
        The result is an array of size (self.landmarks.shape[0], self.new_data.shape[1])
        whose (i, j)-entry is 6 * (1 - (X_i - Y_j)^2 / self.bw^2)^2 * (X_i - Y_j) / self.bw^2 if |X_i - Y_j|
        is less than or equal to self.bw and is 0 otherwise,
        where X_i is the i-th row in landmarks, and Y_j is the j-th row in new_data.
        
    basisfunction_deriv2(new_data)
        Returns the evaluations of the second derivative of the triweight basis function
        with respect to the second argument at new_data.
        The result is an array of size (self.landmarks.shape[0], self.new_data.shape[1])
        whose (i, j)-entry is -6 * (1 - 5 * (X_i - Y_j)^2 / self.bw^2)^2) * (1 - (X_i - Y_j)^2 / self.bw^2) / self.bw^2
...
class SigmoidBasisFunction(BasisFunction):
    ...
    basisfunction_name : str
        The name of the basis function type, i.e., 'Sigmoid'.
    ...
    basisfunction_eval(new_data)
        ...
        The result is an array of size (self.landmarks.shape[0], self.new_data.shape[1])
        whose (i, j)-entry is 1 / (1 + exp ( - (X_i - Y_j) / self.bw)),
        where X_i is the i-th row in landmarks, and Y_j is the j-th row in new_data.
        
    basisfunction_deriv1(new_data)
        Returns the evaluations of the first derivative of the sigmoid basis function
        with respect to the second argument at new_data.
        The result is an array of size (self.landmarks.shape[0], self.new_data.shape[1])
        whose (i, j)-entry is - exp(-(x - y) / self.bw) / (1 + exp(-(x - y) / self.bw))^2 / self.bw,
        where X_i is the i-th row in landmarks, and Y_j is the j-th row in new_data.
        
    basisfunction_deriv2(new_data)
        Returns the evaluations of the second derivative of the sigmoid basis function
        with respect to the second argument at new_data.
        The result is an array of size (self.landmarks.shape[0], self.new_data.shape[1])
        whose (i, j)-entry is the second derivative of the sigmoid basis function with respect to
        the second argument evaluated at (X_i, Y_j),
        where X_i is the i-th row in landmarks, and Y_j is the j-th row in new_data.
